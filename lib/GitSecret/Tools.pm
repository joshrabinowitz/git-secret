package GitSecret::Tools;
use GitSecret::Config;
use Exporter 'import';
our @EXPORT = qw( 
    git_secret_abort 
    git_secret_assert_secrets_dir_is_not_ignored  
    git_secret_check_ignore 
);


sub git_secret_abort {
   my ($config, $message, $exit_code) = @_;
   $exit_code //= 1;    # //= is 'defined or'
   print STDERR "git-secret: abort: $message\n";
   exit($exit_code);
}

sub git_secret_assert_secrets_dir_is_not_ignored {
    my $config = shift;
    my $secret_dir = $config->secret_dir;
    if ( git_secret_check_ignore( $config, $secret_dir ) ) {
        #git_secret_abort( "'$secret_dir' is in .gitignore" );
    }
}

sub git_secret_gitignore_add_pattern {
    my ($config, $pattern) = @_;
    my $gitignore_file_path = git_secret_root_path() . "/.gitignore";
}
...
function gitignore_add_pattern {
  ...
  git_secret_create_gitignore_if_not_created();
...
# VCS
# returns 1 if file is ignored
sub git_secret_check_ignore {
    my ($config, $filename) = @_;

    my $result = `git check-ignore -f "$filename"`;
    my $exit_code = $?;     # see 'perldoc -f backtick'
